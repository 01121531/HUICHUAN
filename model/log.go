...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, group string, requestId string, upstreamRequestId string, proxyId int) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	...
	if proxyId > 0 {
		tx = tx.Where("logs.proxy_id = ?", proxyId)
	}
	if startTimestamp != 0 {
...
func GetUserLogs(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string, proxyId int) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	if logType == LogTypeUnknown {
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string, proxyId int) (stat Stat, err error) {
	...
	if proxyId > 0 {
		tx = tx.Where("proxy_id = ?", proxyId)
		rpmTpmQuery = rpmTpmQuery.Where("proxy_id = ?", proxyId)
	}
